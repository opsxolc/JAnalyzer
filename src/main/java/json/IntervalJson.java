...
public class IntervalJson
{
    ...
    public IntervalJson getIntervalForProcs(Predicate<Integer> procPred) {
    ...
    private void recountAggregated() {
        ...
        times.prod = times.prod_cpu + times.prod_sys + times.prod_io;
        times.exec_time = proc_times.stream().mapToDouble(p -> p.exec_time).sum();
        times.sys_time = proc_times.stream().mapToDouble(p -> p.sys_time).sum();
        times.efficiency = times.prod / times.sys_time; // TODO: ?
        ...
        times.threadsOfAllProcs = proc_times.stream().mapToLong(p -> p.num_threads).sum();
    }
